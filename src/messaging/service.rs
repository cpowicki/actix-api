...
impl MessengerService {
    ...
    pub fn list_topics(&self) -> Vec<&String> {
        // TODO return list of rest::api::Topic's here
    ...
    pub fn register_topic(&mut self, name: String) {
        ...
        let topic = Topic::new(name.clone(), self.db.clone(), rx_msg);
        topic.init();
    ...
    // Clear a topic
    pub async fn clear_topic(&mut self, topic: String) -> Result<()> {
        match self.topics.get_mut(&topic) {
            Some(sender) => sender
                .send(TopicOperation::SendMessage(Message::Clear))
                .await
                .map_err(|_| ErrorInternalServerError(anyhow!("Failed to send message"))),
            None => Err(ErrorNotFound(anyhow!("No topic found"))),
        }
